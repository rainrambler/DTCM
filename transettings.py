...
class TranSettings:
    ...
    def print_info(self):
        print(
            f"Ticket: {self.origArea} to {self.destArea}: {self.dataType} [{self.reason}] ({self.dataVolume} {self.dataUnit}) {self.ticketId}"
        )
...
def create_setting(
    ...
    settings = TranSettings()
    settings.ticketId = str(uuid.uuid4())  # Random
    settings.senderId = get_user_id(sender)
    settings.receiverId = "Bob"
    settings.receiverKey = get_user_id(receiver)
    settings.expectTime = expected_date.isoformat() + "Z"
    settings.expirationTime = expiration_date.isoformat() + "Z"
    settings.origArea = orig_area
    settings.destArea = dest_area
    settings.dataType = data_type
    settings.dataVolume = data_vol
    settings.dataUnit = data_unit
    settings.dataHash = "5usHGst9SANMEViiINEDrZ37UZY="
    settings.reason = reason

    # IMPORTANT!
    settings.approver = get_user_id(approver1)
    return settings
...
def random_datatype() -> str:
    "return a random type of transborder data"
    all_values = ["financial", "payment", "personal data", "privacy"]
    total = len(all_values)
    idx = random.randint(0, total - 1)
    return all_values[idx]
