...
from propset import TransProps, OrgType, find_org
...
class Rules:
    ...
    def print_info(self):
        if self.rule_name == "":
            print(
                f"Allows: {len(self.allow_rules)}, Denies: {len(self.deny_rules)}, Risks: {len(self.risk_rules)}"
            )
        else:
            print(
                f"{self.rule_name}: Allows: {len(self.allow_rules)}, Denies: {len(self.deny_rules)}, Risks: {len(self.risk_rules)}"
            )
    ...
    def judge(self, req: TransProps) -> Decision:
        ...
        for rl in self.allow_rules:
            ...
            if res == Decision.GO:
                return Decision.GO
            if res == Decision.REJECT:
                print(f"Fobidden in an allow rule: {req.trans_props.ticketId}")
                return Decision.REJECT

        for rl in self.risk_rules:
            res = rl.check(req)
            # print(f"DBG: Result: {res} for {req.trans_props.origArea} to {req.trans_props.destArea}")
            if res == Decision.RISK:
                return Decision.RISK
            if res == Decision.REJECT:
                print(f"Fobidden in a risk rule: {req.trans_props.ticketId}")
                return Decision.REJECT
            if res == Decision.GO:
                print(f"Allowed in a risk rule: {req.trans_props.ticketId}")
                return Decision.GO
...
class RuleChecker:
    ...
    # default constructor
    def __init__(self):
        self.gdpr_rules = GdprRules("GDPR")
        self.uk_gdpr_rules = UkGdprRules("UK GDPR")
        self.canada_rules = CanadaRules("Canada")
        self.usa_rules = UsaRules("USA")
        self.vn_rules = VietnamRules("Vietnam")
        self.in_rules = IndianRules("India")
        self.cn_rules = ChinaRules("China")
        self.jp_rules = JapanRules("Japan")
        self.sg_rules = SingaporeRules("Singapore")
        self.ma_rules = MacroRules("Macro")
        self.hk_rules = HKRules("Hong Kong")
    ...
    def judge(self, req: TransProps) -> Decision:
        ...
        match orig_region:
            case Region.EEA:
                des = self.gdpr_rules.judge(req)
            case Region.UK:
                des = self.uk_gdpr_rules.judge(req)
            case Region.CANADA:
                des = self.canada_rules.judge(req)
            case Region.USA:
                des = self.usa_rules.judge(req)
            case Region.VIETNAM:
                des = self.vn_rules.judge(req)
            case Region.INDIA:
                des = self.in_rules.judge(req)
            case Region.CHINA:
                des = self.cn_rules.judge(req)
            case Region.JAPAN:
                des = self.jp_rules.judge(req)
            case Region.SINGAPORE:
                des = self.sg_rules.judge(req)
            case Region.MACRO:
                des = self.ma_rules.judge(req)
            case _:
                # TODO implementation
                print(
                    f"INFO: Cannot find checker for region: {req.trans_props.origArea}"
                )
    ...
    def check(self, req: TransProps) -> Decision:
        ...
        match descl:
            ...
            case "the united states" | "united states" | "usa":
                return eu_to_us(req)
            case _:
                print(f"GDPR: Ticket out of adequacy: {req.trans_props.ticketId}")
                return Decision.TBD
...
def eu_to_us(req: TransProps) -> Decision:
    ...
    if reasonl.find("privacy shield") != -1:
        ...
        print(
            f"Ticket has an invalid reason (privacy shield): {req.trans_props.ticketId}"
        )
        return Decision.TBD
    ...
    def check(self, req: TransProps) -> Decision:
        ...
        match descl:
            ...
            case "the united states" | "united states" | "usa":
                return uk_to_us(req)
            case _:
                print(f"UK: Ticket out of adequacy: {req.trans_props.ticketId}")
                return Decision.TBD
...
class RuleCaOPC(RiskRule):
    ...
    def check(self, req: TransProps) -> Decision:
        dest = find_region(req.trans_props.destArea)
        # print(f"{dest} found for {req.trans_props.destArea}")
...
class RuleInBlacklist(DenyRule):
    ...
    def check(self, req: TransProps) -> Decision:
        "Article 16 clause 1"
        blacklist = ["pakistan"]  # The banned regions' name
        dest = req.trans_props.destArea.lower()
...
class ChinaRules(Rules):
    def __init__(self, nm: str):
        ...
        self.add_risk(RuleCnCIIsSimple())
...
class RuleCnCIIs(RiskRule):
    ...
    def check(self, req: TransProps) -> Decision:
        ...
        if is_pii(req.trans_props.dataType) or is_cn_important(
            req.trans_props.dataType
        ):
...
class RuleCnCIIsSimple(RiskRule):
    "Facilitate and regulate regulations for cross-border data flows, March 2024, Article 7"
